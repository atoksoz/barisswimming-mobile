...
/// Eğitmen anasayfa — Randevu `today-summary`: bugünkü yoklama ve bugünün özeti diyalogları.
/// Yüzme / müzik okulu vb. tüm trainer panellerinde paylaşılır.
class TrainerTodayDashboardDialogs {
  TrainerTodayDashboardDialogs._();

  static String _singleLineTitle(String raw) =>
      raw.replaceAll('\n', ' ').trim();

  /// [showAttendanceLessonName]: yalnız Müzik Okulum’da `true`; yüzme vb. için `false`.
  static Future<void> showTodayAttendance(
    BuildContext context,
    TrainerTodayDashboardModel model, {
    bool showAttendanceLessonName = false,
  }) async {
    ...
    await showDialog<void>(
      ...
      builder: (_) => SummaryPopupWidget(
        ...
        subtitle: model.date.isNotEmpty
            ...
            : null,
        items: items,
        itemBuilder: (theme, m) => _attendanceItem(
          theme,
          labels,
          m,
          showLessonName: showAttendanceLessonName,
        ),
      ),
    );
  }
  ...
  static Widget _attendanceItem(
    ...
    bool showLessonName = false,
  }) {
    final name = m['member_name']?.toString() ?? '';
    final lesson = m['lesson_name']?.toString() ?? '';
    final time = m['plan_time']?.toString() ?? '';
    final note = m['note']?.toString() ?? '';
    final timeLine =
        time.isNotEmpty ? '${labels.time}: $time' : '';
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (showLessonName && lesson.isNotEmpty) ...[
                  SizedBox(height: theme.panelTightVerticalGap * 2),
                  Text(
                    lesson,
                    style:
                        theme.textSmallNormal(color: theme.defaultGray600Color),
                    maxLines: 2,
