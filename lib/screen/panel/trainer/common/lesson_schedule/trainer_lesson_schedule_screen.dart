...
import 'package:e_sport_life/screen/panel/trainer/muzik-okulum/muzik_okulum_trainer_lesson_roster_attendance_screen.dart';
...
typedef _TrainerLessonFormPopResult = ({
  int saved,
  int failed,
...
/// Eğitmen ortak haftalık grup ders programı — Randevu takvim verisi (**Grup Dersleri**, `GroupLesson`).
class TrainerLessonScheduleScreen extends StatefulWidget {
  const TrainerLessonScheduleScreen({
    super.key,
    ...
    /// [false] → AppBar’da ders ekleme yok, kartlarda düzenle/sil yok (ör. müzik okulu eğitmen).
    this.allowScheduleMutations = true,
    /// Müzik okulu: takvimden yoklama → Randevu kayıtlı öğrenci listesi (Fitiz toplu yoklama).
    this.servicePlanRosterAttendance = false,
    /// Müzik okulu: kontenjan rozetine dokununca öğrenci listesi popup’ı.
    this.showRegisteredParticipantsOnCapacityTap = false,
    /// [true] → ders kartındaki yoklama al kısayolu ve yoklama ekranı açılmaz.
    this.skipAttendanceTaking = false,
  });
  ...
  /// Grup ders planı üzerinde ekle / düzenle / sil; [pickLessonForAttendance] iken zaten kapalıdır.
  final bool allowScheduleMutations;

  /// Randevu `service-plans/.../attendance` ile kayıtlı öğrenci yoklaması (QR üye seçimi yerine).
  final bool servicePlanRosterAttendance;

  /// Kontenjan (sağ üst) tıklanınca [TrainerLessonScheduleParticipantsDialog].
  final bool showRegisteredParticipantsOnCapacityTap;

  /// Geçici: müzik okulu eğitmeninde yoklama alma akışını kapatır.
  final bool skipAttendanceTaking;

  @override
  State<TrainerLessonScheduleScreen> createState() =>
      _TrainerLessonScheduleScreenState();
}

class _TrainerLessonScheduleScreenState
    extends State<TrainerLessonScheduleScreen> {
  ...
  void _openAttendanceWithLesson(TrainerScheduleCalendarEventModel data) {
    if (widget.skipAttendanceTaking) {
      return;
    }
    if (data.servicePlanId <= 0) return;
    if (widget.servicePlanRosterAttendance) {
      Navigator.of(context).push<void>(
        MaterialPageRoute<void>(
          builder: (_) => MuzikOkulumTrainerLessonRosterAttendanceScreen(
            initialEvent: data,
          ),
        ),
      );
  ...
  Future<void> _handleLessonFormPopResult(Object? raw) async {
    if (raw is! _TrainerLessonFormPopResult) return;
    final result = raw;
  ...
  Widget build(BuildContext context) {
    ...
    final title = _locksToTodayForAttendance
        ...
        : labels.trainerLessonScheduleScreenTitle;

    return Scaffold(
      appBar: TopAppBarWidget(
        title: title,
        actions: (_locksToTodayForAttendance || !widget.allowScheduleMutations)
            ? null
            : [
                IconButton(
                  tooltip: labels.add,
      ...
      bottomNavigationBar: _locksToTodayForAttendance
          ? null
          : BottomNavigationBarWidget(tab: NavTab.home),
    );
  }
      ...
      itemBuilder: (context, index) {
        ...
        return TrainerGroupLessonScheduleCard(
          ...
          onTap: pick && data.servicePlanId > 0
              ...
              : null,
          onCapacityBadgeTap: widget.showRegisteredParticipantsOnCapacityTap
              ? () => TrainerLessonScheduleParticipantsDialog.show(context, data)
              : null,
          bottomActions:
              pick ? null : _lessonCardBottomActions(theme, labels, data, showAttendanceOnCard),
        );
      },
    );
  }

  /// Düzenle / sil yalnız [allowScheduleMutations] açıkken; yoklama kısayolu aynı kalır.
  Widget? _lessonCardBottomActions(
    BaseTheme theme,
    AppLabels labels,
    TrainerScheduleCalendarEventModel data,
    bool showAttendanceOnCard,
  ) {
    if (_locksToTodayForAttendance) return null;
    final hasAttendance = showAttendanceOnCard &&
        data.servicePlanId > 0 &&
        !widget.skipAttendanceTaking;
    final allowMut = widget.allowScheduleMutations;
    if (!hasAttendance && !allowMut) return null;
    return Row(
      crossAxisAlignment: CrossAxisAlignment.center,
      children: [
        if (hasAttendance) _buildLessonCardAttendancePill(theme, labels, data),
        if (hasAttendance || allowMut) const Spacer(),
        if (allowMut) ...[
