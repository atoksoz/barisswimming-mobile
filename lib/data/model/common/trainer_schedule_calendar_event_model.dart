...
class TrainerScheduleCalendarEventModel {
  ...
  /// O günkü rezervasyon / yoklama satırları (`reservations`).
  final List<TrainerCalendarReservationRowModel> reservations;

  /// Derse kayıtlı öğrenciler (`enrollments`).
  final List<TrainerCalendarEnrollmentRowModel> enrollments;

  /// api-system paket düşümü — sunucu yoklama kaydında kullanılır.
  ...
  factory TrainerScheduleCalendarEventModel.fromJson(
    ...
    List<TrainerCalendarReservationRowModel> reservations = const [];
    if (rawRes is List) {
      reservations = rawRes
          .map((e) => TrainerCalendarReservationRowModel.fromJson(
                Map<String, dynamic>.from(e as Map),
              ))
          .toList();
    }

    final rawEnr = json['enrollments'];
    List<TrainerCalendarEnrollmentRowModel> enrollments = const [];
    if (rawEnr is List) {
      enrollments = rawEnr
          .map((e) => TrainerCalendarEnrollmentRowModel.fromJson(
                Map<String, dynamic>.from(e as Map),
              ))
          .toList();
    }
    ...
    final tp = json['track_payment'] ?? json['trackPayment'];
    final trackPayment = tp == null
        ? true
        : (tp == true ||
            tp == 1 ||
            tp == '1' ||
            (tp is String && tp.toLowerCase() == 'true'));
