...
class LoyaltyService {
  static Future<double?> getBalance({
    ...
    try {
      final url = KantincimUrlConstants.getLoyaltyBalanceUrl(kantincimUrl);
      final response = await RequestUtil.get(url, token: token);

      if (response != null && response.statusCode == 200) {
        final jsonMap = json.decode(response.body);
        if (jsonMap['status'] == 200 || jsonMap['status'] == true) {
      ...
      print('Error getting loyalty balance: $e');
      return null;
    }
  }
  ...
  static Future<Map<String, dynamic>?> getLogs({
    ...
    try {
      final baseUrl = KantincimUrlConstants.getLoyaltyLogsUrl(kantincimUrl);
      ...
      print('Error getting loyalty logs: $e');
      return null;
    }
  }
}
